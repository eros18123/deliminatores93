...
class CustomDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        media_layout.addWidget(self.edit_button)
        ...
        # --- CONEXÃO PARA EDIÇÃO EM TEMPO REAL ---
        ...
        # Adicionamos o divisor de campos (fields_splitter) com um fator de
        # esticamento (stretch) de 1. Isso diz ao layout para dar todo o
        # espaço vertical extra a este widget, fazendo-o expandir.
        ...
        # --- INÍCIO DA MODIFICAÇÃO: CONTAINER PARA FERRAMENTAS ---
        self.formatting_tools_widget = QWidget()
        formatting_tools_layout = QVBoxLayout(self.formatting_tools_widget)
        formatting_tools_layout.setContentsMargins(0, 0, 0, 0)
        ...
        formatting_tools_layout.addLayout(btn_layout)
        ...
        formatting_tools_layout.addLayout(search_layout)
        ...
        formatting_tools_layout.addLayout(cloze_layout)
        
        bottom_layout.addWidget(self.formatting_tools_widget)
        # --- FIM DA MODIFICAÇÃO: CONTAINER PARA FERRAMENTAS ---
        ...
        # --- INÍCIO DA MODIFICAÇÃO: BOTÃO PARA OCULTAR FERRAMENTAS ---
    ...
    def retranslate_ui(self):
        ...
        # --- INÍCIO DA MODIFICAÇÃO: TRADUÇÃO DO NOVO BOTÃO ---
        is_formatting_visible = self.formatting_tools_widget.isVisible()
    ...
    # =============================================================================
    # FUNÇÃO DE DIVISÃO CORRIGIDA
    # =============================================================================

    def _get_split_parts(self, line_text: str) -> list[str]:
        """
        Divide uma linha de texto em partes com base nos delimitadores ativos,
        ignorando os delimitadores que estão dentro de tags HTML.
        """
        active_delimiters = [chk.simbolo for chk in self.chk_delimitadores.values() if chk.isChecked()]
        ...
        parts = []
        current_part = ""
        tag_level = 0  # Contador para saber se estamos dentro de uma tag (lida com tags aninhadas)
        
        # Itera por cada caractere da linha
        for char in line_text:
            # Se encontrar '<', incrementa o nível, pois entramos em uma tag
            if char == '<':
                tag_level += 1
            
            # Se o caractere for o delimitador E o nível da tag for 0 (ou seja, não estamos dentro de nenhuma tag)
            if char == delimiter and tag_level == 0:
                # Finaliza a parte atual e a adiciona na lista
                parts.append(current_part.strip())
                # Começa uma nova parte
                current_part = ""
            else:
                # Se não for um ponto de divisão, apenas adiciona o caractere à parte atual
                current_part += char

            # Se encontrar '>', decrementa o nível, pois saímos de uma tag
            if char == '>':
                tag_level = max(0, tag_level - 1) # Garante que o nível nunca fique negativo
        
        # Adiciona a última parte que sobrou após o loop
        parts.append(current_part.strip())
    ...
    def _check_for_state_reset(self):
        """
        Reseta o modo de preview se o usuário limpar o editor após carregar cards.
        Isso força a volta do modo "edição" para o modo "criação" de preview.
        """
        # Se estávamos no modo de edição (a lista de IDs tem conteúdo)
        # e o usuário apagou todo o texto...
        if self.shown_note_ids and not self.txt_entrada.toPlainText():
            # ...então limpamos a lista de IDs para sair do modo de edição.
            self.shown_note_ids.clear()
            
            # Também garantimos que o modo de edição seja desativado visualmente.
            if self.edit_mode:
                self.toggle_edit_mode()
            
            # Forçamos a atualização do preview para limpar a tela.
            self.update_preview()
    ...
    def update_preview(self):
        try:
            ...
            # Garante que a linha atual é válida
            ...
            # Verifica se um tipo de nota (modelo) está selecionado
            if not self.lista_notetypes.currentItem():
            ...
            # CASO 1: MODO DE EDIÇÃO (Preview de um card existente que foi carregado)
            if self.shown_note_ids and 0 <= current_line < len(self.shown_note_ids):
                nid = self.shown_note_ids[current_line]
                note = mw.col.get_note(nid)

            # CASO 2: MODO DE CRIAÇÃO (Preview de um novo card que está sendo digitado)
            else:
                ...
                # Cria uma nota temporária em memória (não salva no Anki)
                note = mw.col.new_note(model)
                parts = self._get_split_parts(line_text)
                
                # Preenche os campos da nota temporária com base no mapeamento
            ...
            # --- LÓGICA DE RENDERIZAÇÃO (COMUM PARA AMBOS OS CASOS) ---
            if not note:
                self.preview_widget.setHtml("") # Limpa se não houver nota
                return
            ...
            <div class="preview-section" style="border: 1px solid #4a4a4a; border-radius: 8px; margin: 10px; overflow: hidden;">
                ...
                <div id="preview-front" class="preview-content" style="padding: 15px; overflow-y: auto;">{question_html}</div>
            </div>
            <div class="preview-section" style="border: 1px solid #4a4a4a; border-radius: 8px; margin: 10px; overflow: hidden;">
                ...
                <div id="preview-back" class="preview-content" style="padding: 15px; overflow-y: auto;">{answer_html}</div>
            </div>
            """
    ...
    def show_all_cards(self):
        # --- DESATIVA O MODO DE EDIÇÃO AO MOSTRAR NOVOS CARDS ---
        if self.edit_mode:
            self.toggle_edit_mode()
        ...
        card_lines = []
        card_tags_lines = [] # --- INÍCIO DA CORREÇÃO ---
        self.card_notetypes = []
        self.shown_note_ids.clear()
        ...
        for nid in note_ids:
            ...
            # Captura as tags da nota e as adiciona à lista
            tags = note.string_tags().strip()
            card_tags_lines.append(tags)
        ...
        final_text = clean_html_tags(final_text)
        self.txt_entrada.blockSignals(True)
        self.txt_entrada.setPlainText(final_text)
        self.txt_entrada.blockSignals(False)

        # Popula o campo de texto das etiquetas
        final_tags = "\n".join(card_tags_lines)
        self.txt_tags.blockSignals(True)
        self.txt_tags.setPlainText(final_tags)
    ...
    # --- FUNÇÕES PARA EDIÇÃO EM TEMPO REAL (CORRIGIDAS E MELHORADAS) ---
    ...
    def toggle_edit_mode(self):
        ...
        if self.edit_mode:
            self.edit_button.setText(self._t("Parar Edição"))
            self.edit_button.setStyleSheet("background-color: #ffdddd; color: black;") # Vermelho claro
            self.show_button.setEnabled(False)
            # --- AVISO SUTIL NA BARRA DE STATUS (SEM POP-UP) ---
            ...
            self.edit_button.setText(self._t("Editar"))
            self.edit_button.setStyleSheet("") # Reseta o estilo
    ...
    def _apply_real_time_edit(self):
        ...
        try:
            # Usa o cursor do editor de cards como referência para a linha
            ...
            # --- LÓGICA PARA ATUALIZAR OS CAMPOS DO CARD (JÁ EXISTENTE) ---
            all_lines = self.txt_entrada.toPlainText().split('\n')
            ...
            # --- INÍCIO DA NOVA LÓGICA PARA ATUALIZAR AS ETIQUETAS ---
            all_tags_lines = self.txt_tags.toPlainText().split('\n')
            if line_number < len(all_tags_lines):
                # Pega a linha de tags correspondente
                tags_for_card_str = all_tags_lines[line_number].strip()
                # Converte a string de tags (separada por vírgula ou espaço) em uma lista
                # e atribui à nota. O Anki lida com a limpeza e formatação.
                note.tags = mw.col.tags.split(tags_for_card_str)
            # --- FIM DA NOVA LÓGICA ---

            note.flush() # Salva as alterações dos campos e das etiquetas no banco de dados
            
            # --- LÓGICA PARA ATUALIZAR A TELA DE REVISÃO (JÁ EXISTENTE) ---
    ...
    # --- INÍCIO DA MODIFICAÇÃO: FUNÇÃO PARA OCULTAR FERRAMENTAS ---
    def toggle_formatting_tools(self):
        is_visible = self.formatting_tools_widget.isVisible()
        self.formatting_tools_widget.setVisible(not is_visible)
        if is_visible:
